...
- `network_configuration_id` - (Optional) The ID of a `github_organization_network_configuration` to assign to the runner group, so GitHub-hosted runners in the group run inside your private network. Removing this attribute currently replaces the runner group because the GitHub client library cannot encode the explicit null required to clear the assignment.
