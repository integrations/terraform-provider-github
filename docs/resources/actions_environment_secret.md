...
To keep the secret out of state entirely, set `value_wo` instead of `value`. It is a write-only argument, sent to GitHub on apply but never persisted. Terraform cannot detect a change to a value it does not store, so `value_wo_version` must be incremented whenever the value changes.
...
- `value_wo` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Plaintext value to be encrypted, never persisted to state.
- `value_wo_version` (Number) Version of `value_wo`. Increment this to write a changed `value_wo` to GitHub, as write-only values cannot be detected in state.
