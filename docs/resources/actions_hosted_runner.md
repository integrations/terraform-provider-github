...
~> **Note:** Private networking is configured on the runner group rather than on the hosted runner. Assign a `github_organization_network_configuration` to a runner group through `github_actions_runner_group.network_configuration_id`, then place the hosted runner in that group.
