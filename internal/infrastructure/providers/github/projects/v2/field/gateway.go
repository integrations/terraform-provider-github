...
func (gateway *Gateway) Create(ctx context.Context, input application.CreateInput) (application.Result, error) {
	...
	if err := gateway.client.Mutate(ctx, &mutation, variables, nil); err != nil {
		return application.Result{}, projectgraphql.Error("creating Projects V2 field", err)
	}
	return resultFromNode(mutation.CreateProjectV2Field.Field)
}

func (gateway *Gateway) Get(ctx context.Context, id string) (application.Result, error) {
	operation := fmt.Sprintf("querying Projects V2 field %q", id)
	var query struct {
...
func (gateway *Gateway) Update(ctx context.Context, input application.UpdateInput) (application.Result, error) {
	...
	if err := gateway.client.Mutate(ctx, &mutation, variables, nil); err != nil {
		return application.Result{}, projectgraphql.Error(fmt.Sprintf("updating Projects V2 field %q", input.ID), err)
	}
	return resultFromNode(mutation.UpdateProjectV2Field.Field)
}
