...
func resultFromNode(projectID string, value teamNode) application.Result {
	return application.Result{ProjectID: projectID, TeamID: string(value.ID), DatabaseID: int(value.DatabaseID), Organization: string(value.Organization.Login), Slug: string(value.Slug)}
}
