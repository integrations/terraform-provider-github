...
func resultFromNode(projectID string, value repositoryNode) application.Result {
	return application.Result{ProjectID: projectID, RepositoryID: string(value.ID), DatabaseID: int(value.DatabaseID), Owner: string(value.Owner.Login), Name: string(value.Name)}
}
