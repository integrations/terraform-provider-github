	...
	t.Run("queries environment deployment policies", func(t *testing.T) {
		...
		config := fmt.Sprintf(`
			...
			data "github_repository_environment_deployment_policies" "all" {
				...
				depends_on = [
					github_repository_environment_deployment_policy.branch,
					github_repository_environment_deployment_policy.tag,
				]
			}
	`, randomID)
		check := resource.ComposeTestCheckFunc(
			...
			resource.TestCheckResourceAttr("data.github_repository_environment_deployment_policies.all", "policies.0.pattern", "foo"),
			resource.TestCheckResourceAttr("data.github_repository_environment_deployment_policies.all", "policies.1.type", "tag"),
			resource.TestCheckResourceAttr("data.github_repository_environment_deployment_policies.all", "policies.1.pattern", "bar"),
		)
