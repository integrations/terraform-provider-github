...
func testAccOrganizationBlockDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		res, err := conn.Organizations.UnblockUser(context.TODO(), ownerName, username)
...
func testAccCheckOrganizationBlockExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		blocked, _, err := conn.Organizations.IsBlocked(context.TODO(), ownerName, username)
		if err != nil {
			return err
		}
		if !blocked {
			return fmt.Errorf("not blocked: %s %s", ownerName, username)
		}
		return nil
	}
}
