...
func resourceGithubRepositoryCollaboratorRead(d *schema.ResourceData, meta interface{}) error {
	...
	invitation, err := findRepoInvitation(client, ctx, ownerName, repoName, username)
	if err != nil {
...
func resourceGithubRepositoryCollaboratorDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		_, err = client.Repositories.DeleteInvitation(ctx, ownerName, repoName, *invitation.ID)
		return err
	}

	log.Printf("[DEBUG] Deleting repository collaborator: %s (%s/%s)",
		username, ownerName, repoName)
	_, err = client.Repositories.RemoveCollaborator(ctx, ownerName, repoName, username)
	return err
}
