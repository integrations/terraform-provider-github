...
func TestAccGithubEnterpriseCustomPropertiesResource(t *testing.T) {
	t.Run("creates basic custom property without error", func(t *testing.T) {
	...
	t.Run("creates custom property using allowed_values", func(t *testing.T) {
		randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlpha)
		propertyName := fmt.Sprintf("test-%s", randomID)
		...
		check := resource.ComposeTestCheckFunc(
			resource.TestCheckResourceAttr("data.github_enterprise_custom_properties.test", "allowed_values.#", "3"),
			resource.TestCheckResourceAttr("data.github_enterprise_custom_properties.test", "value_type", "single_select"),
		)
